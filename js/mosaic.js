...
Ext.define('Wise.Filters.Filter', {
    render: function() {}
});
...
/* , this.width * this.width / this.height, this.height * this.height / this.width */
Ext.define('Wise.Filters.Pixelate.MosaicAnimator', {
	...
	constructor: function(img) {
		...
		this.fps = parseInt(this.$img.attr('data-fps'));
		this.maxSize = parseInt(this.$img.attr('data-max-size'));
		...
		this.width =  this.img.width || this.img.offsetWidth;
		this.height = this.img.height || this.img.offsetHeight;
		...
		this.src.getContext("2d").drawImage(this.img, 0, 0);
		
		this.scene = document.createElement("canvas");
		this.scene.width = this.width;
		this.scene.height = this.height;
		this.sceneContext = this.scene.getContext("2d");
		this.sceneContext.drawImage(this.img, 0, 0);
		
		this.framebuffer = document.createElement("canvas");
		this.framebuffer.width = this.width;
		this.framebuffer.height = this.height;
		
		this.context = this.framebuffer.getContext("2d");
		...
		this.frames.push(this.renderAnimationFrame(1));
		...
		(function preRenderFrames() {
			if (i < (self.maxSize + 1)) { 
				handle = requestAnimationFrame(preRenderFrames);
				self.frames.push(self.renderAnimationFrame(i++));
			}
			else {
		...
		this.$container.on('mouseenter', this.bind(this.onMouseEnter, this))
					   .on('mouseleave', this.bind(this.onMouseLeave, this))
					   .append(this.scene);
	...
	renderAnimationFrame: function(size) {
		if (size < 1) {
			return null;
		}
		...
		if (size == 1) {
			...
			for (x = 0; x < this.height; x += size) {
				for (y = 0; y < this.width; y += size) {
					r = this.data[((x * (this.width * 4)) + (y * 4))];
					g = this.data[((x * (this.width * 4)) + (y * 4)) + 1];
					b = this.data[((x * (this.width * 4)) + (y * 4)) + 2];
					
					this.context.fillStyle = 'rgb('+r+','+g+','+b+')';
					this.context.fillRect(y, x, size, size);
				}
			}
		}
		...
		result.getContext("2d").drawImage(this.framebuffer, 0, 0);
	...
	animationLoop: function() {
		...
		return (function renderLoop() {
			self.handle = requestAnimationFrame(renderLoop);
			
			var now = new Date().getTime(),
				deltaTime = now - time;

			if (deltaTime > interval) {
				time = now - (deltaTime % interval);
				
				if (self.effect === 'on' && self.current < self.frames.length) {
					self.sceneContext.drawImage(self.frames[self.current++], self.sx, self.sy);
				}
				else if (self.effect === 'off' && self.current > (self.minSize-1)) {
					self.sceneContext.drawImage(self.frames[--self.current], self.sx, self.sy);
				}
				else {
					self.effect = '';
...
Wise.Filters.Pixelate.MosaicAnimator.create = function(el) {
	return new Wise.Filters.Pixelate.MosaicAnimator(el);
}
